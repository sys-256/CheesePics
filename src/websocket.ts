...
export const startWSServer = () => {
    const ws = new WebSocketServer(
        ...
        async () => {
            console.log(
                `Listening for WebSocket connections on port ${config.port.websocket}`,
            );
        },
    );
        ...
        forge.pki.rsa.generateKeyPair(
            ...
            async (error: Error, keypair: forge.pki.rsa.KeyPair) => {
                if (error) {
                    console.error(error);
                    socket.send(
                        `ERR;;SERVER;;An error occurred while generating a keypair.`,
                    );
                    socket.close();
                    return;
                }
                ...
                socket.on("message", async (message_buffer) => {
                    ...
                    let clientPublickey;
                    try {
                        clientPublickey = forge.pki.publicKeyFromPem(
                            message_encrypted.split("::")[0],
                        );
                    } catch (error) {
                        socket.send(
                            `ERR;;CLIENT;;Please send a valid public key.`,
                        );
                        socket.close();
                        return;
                    }
                    const new_message: string = message_encrypted.slice(
                        message_encrypted.indexOf("::") + 2,
                    );

                    // Make sure it's a valid encrypted message
                    let pfp_data: string | undefined;
                    if (
                        new_message.length !== 128 &&
                        new_message.includes("::")
                    ) {
                        if (new_message.includes("::")) {
                            try {
                                pfp_data = new_message.slice(
                                    new_message.indexOf("::") + 2,
                                );
                            } catch (err) {
                                socket.send(
                                    `ERR;;CLIENT;;Please send a valid encrypted message.`,
                                );
                                socket.close();
                                return;
                            }
                    ...
                    const message: string[] = keypair.privateKey
                        .decrypt(new_message.split("::")[0])
                        .split(";;");

                    // Check if the message contains at least 2 parts
                    if (message.length < 1) {
                        socket.send(
                            `ERR;;CLIENT;;Specify at least a public key and method.`,
                        );
                        socket.close();
                        return;
                    }
                    ...
                    if (message[0] === "REGI") {
                        ...
                        if (message.length !== 3) {
                            socket.send(
                                `ERR;;CLIENT;;Specify (only) a method, username and password.`,
                            );
                            socket.close();
                            return;
                        }
