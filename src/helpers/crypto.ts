...
class base64 {
    ...
    decode(): Promise<string> {
        return new Promise((resolve, reject) => {
            const result = Buffer.from(this.message, "base64").toString(
                "utf-8",
            );
            if (!result) {
    ...
    encode(): Promise<string> {
        return new Promise((resolve, reject) => {
            const result = Buffer.from(this.message, "utf-8").toString(
                "base64",
            );
            if (!result) {
                reject("Error decoding base64.");
...
function pbkdf2(password: string, salt: string): Promise<string> {
    return new Promise((resolve, reject) => {
        crypto.pbkdf2(
            ...
            config.pbkdf2.keyLength,
            config.pbkdf2.algorithm,
            (error, key) => {
                if (error) {
                    reject(error);
                    return;
...
function generateSalt(passwordLength: number): Promise<string> {
    return new Promise((resolve, reject) => {
        crypto.randomBytes(
            passwordLength + Math.floor(Math.random() * 30 + 10) / 2,
            (error, salt) => {
                if (error) {
                    reject(error);
                    return;
