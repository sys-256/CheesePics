...
const addUserToDatabase = async (username: string, password: string, salt: string): Promise<string[]> => {
    return new Promise((resolve, reject) => {
        database.query(`INSERT INTO login (username, password, salt, liked) VALUES ('${username}', '${password}', '${salt}', '0');`, (error, results) => {
            if (error) {
                reject(error);
                return;
            }
...
const checkLikeExists = async (username: string, image: string): Promise<boolean> => {
    return new Promise((resolve, reject) => {
        database.query(`SELECT liked FROM login WHERE username='${username}';`, (error, results) => {
            if (error) {
                reject(error);
                return;
            }
...
const addLikeToDatabase = async (username: string, image: string): Promise<boolean> => {
    return new Promise((resolve, reject) => {
        database.query(`UPDATE login SET liked=CONCAT(liked, ',${image}') WHERE username='${username}';`, (error, results) => {
            if (error) {
                reject(error);
                return;
            }
...
const removeLikeFromDatabase = async (username: string, image: string): Promise<boolean> => {
    return new Promise((resolve, reject) => {
        database.query(`SELECT liked from login WHERE username='${username}';`, (error, results) => {
            if (error) {
                reject(error);
            ...
            const result = results[0].liked.split(`,`);
            const newResult = result.filter((item: string) => item !== image);
            database.query(`UPDATE login SET liked='${newResult}' WHERE username='${username}';`, (error, results) => {
                if (error) {
                    reject(error);
                    return;
                }
...
export { addUserToDatabase, checkUserExistsInDB, getSaltFromDB, getPasswdByUsernameFromDB, getRandomImage, checkImageExists, checkLikeExists, addLikeToDatabase, removeLikeFromDatabase };