...
export const uploadAvatar = async (
        ...
        .catch((error) => {
            ...
            socket.send(
                clientPublickey.encrypt(
                    `ICON;;ERR;;SERVER;;An error occurred while checking if the user exists.`,
                ),
            );
            return;
        });
    if (!result) {
        socket.send(
            clientPublickey.encrypt(
                `ICON;;ERR;;CLIENT;;The username or password is incorrect.`,
            ),
        );
        return;
    }
    ...
    const salt = await helper.mariadb
        ...
        .catch((error) => {
            ...
            socket.send(
                clientPublickey.encrypt(
                    `ICON;;ERR;;SERVER;;An error occurred while getting the salt from the database.`,
                ),
            );
            return "";
        });
    ...
    const username = await new helper.crypto.base64(message[1])
        ...
        .catch((error) => {
            socket.send(
                clientPublickey.encrypt(
                    `ICON;;ERR;;SERVER;;An error occurred while decoding the username.`,
                ),
            );
            return "";
        });
    const password = await new helper.crypto.base64(message[2])
        ...
        .catch((error) => {
            socket.send(
                clientPublickey.encrypt(
                    `ICON;;ERR;;SERVER;;An error occurred while decoding the password.`,
                ),
            );
            return "";
        });
    ...
    const password_compare = await helper.crypto
        ...
        .catch((error) => {
            ...
            socket.send(
                clientPublickey.encrypt(
                    `ICON;;ERR;;SERVER;;An error occurred while hashing the password.`,
                ),
            );
            return "";
        });
    ...
    const db_password = await helper.mariadb
        ...
        .catch((error) => {
            ...
            socket.send(
                clientPublickey.encrypt(
                    `ICON;;ERR;;SERVER;;An error occurred while getting the username and password from the database.`,
                ),
            );
            return;
        });
